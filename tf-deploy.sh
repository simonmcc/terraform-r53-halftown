...
check_environment_is_set()
{
    ...
    [ -z "$TF_REMOTESTATE_BUCKET" ] && { log "Need to set TF_REMOTESTATE_BUCKET"; DO_ERROR=1; }
    ...
    log "TF_REMOTESTATE_BUCKET: ${TF_REMOTESTATE_BUCKET}"
...
set_environment()
{
    ...
    if [ -f ${DEPLOY_KIT}/.tf-deploy ] ; then
      source ${DEPLOY_KIT}/.tf-deploy
    fi
}
...
setup_tf_remote()
{
  case ${TF_REMOTESTATE_BACKEND} in
    S3)
      aws s3 ls s3://${TF_REMOTESTATE_BUCKET} || aws s3 mb s3://${TF_REMOTESTATE_BUCKET} || aws s3api put-bucket-versioning --bucket ${TF_REMOTESTATE_BUCKET} --versioning-configuration Status=Enabled
      (cd ${DEPLOY_KIT} ; terraform remote config -backend=S3 -backend-config="bucket=${TF_REMOTESTATE_BUCKET}" \
                                                              -backend-config="key=${TF_BACKEND_KEY}" \
                                                              -backend-config="acl=bucket-owner-full-control")
      ;;
    SWIFT)
      (cd ${DEPLOY_KIT} ; terraform remote config -backend=swift -backend-config="path=${TF_REMOTESTATE_BUCKET}")
      ;;
    *)
      log "ERROR: ${TF_REMOTESTATE_BACKEND} is an unsupported TF_REMOTESTATE_BACKEND type"
      exit 2
      ;;
  esac
}
